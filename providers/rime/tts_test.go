...
func planFor(now time.Time, endpoint string) protocol.SessionPlan {
	return protocol.SessionPlan{
		...
		Reservation:  protocol.Reservation{ID: "res_rime", LeaseDurationSeconds: 60, LeaseExpiresAt: now.Add(time.Minute), Concurrency: protocol.ConcurrencyReservation{LeaseID: "conc_rime", Slots: 1}, Usage: protocol.UsageReservation{Unit: protocol.UsageUnitCharacters, AuthorizedUnits: 4_000}},

...
func TestSttRouteSupportFailsClosedByOptionAndProvider(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "assemblyai streams speaker labels", provider: "assemblyai", model: "universal-3-5-pro",
			options: &protocol.SttOptions{Diarization: boolPointer(true)}, refused: false},
		{name: "assemblyai voice focus serves noise reduction", provider: "assemblyai", model: "universal-3-5-pro",
			options: &protocol.SttOptions{NoiseReduction: boolPointer(true)}, refused: false},
		{name: "assemblyai forwards its voice focus knobs", provider: "assemblyai", model: "universal-3-5-pro",
			options: &protocol.SttOptions{ProviderOptions: map[string]map[string]any{
				"assemblyai": {"voice_focus": "far-field", "voice_focus_threshold": 0.9, "max_speakers": 2},
			}}, refused: false},
		...
		{name: "no streaming route returns word timestamps", provider: "deepgram", model: "nova-3",
			options: &protocol.SttOptions{WordTimestamps: boolPointer(true)}, refused: true, option: "word_timestamps"},
		{name: "unknown provider refuses word timestamps by name", provider: "cartesia", model: "ink-2",
			options: &protocol.SttOptions{WordTimestamps: boolPointer(true)}, refused: true, option: "word_timestamps"},
